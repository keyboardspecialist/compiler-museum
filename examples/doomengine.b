// doomengine — Doom-style 3D engine library.
//
// Lifted from 61-doom-sf2 and refactored into a reusable engine. All
// renderer / WAD-loader / BSP / sprite / sky / door / light / HUD /
// automap / music plumbing lives here, behind a small API in
// g/doomengine.h. The consumer drives input and the main loop; the
// engine owns rendering and per-frame state.
//
// See g/doomengine.h for the full surface. Example consumer:
//   site/examples/62-doom-engine.b
//
// Resolution + camera constants come from ENGINE_W / ENGINE_H /
// ENGINE_HORIZON / ENGINE_NEAR / ENGINE_EYE_H in the header.
...
// TRUE if buf[start..end-1] ends in ".sf2".
LET ends_with_sf2(buf, start, end) = VALOF
{ LET n = end - start
  IF n < 4 RESULTIS FALSE
  RESULTIS lc(buf % (end - 4)) = '.' &
           lc(buf % (end - 3)) = 's' &
...
// Look through the asset list for the first .sf2 name and hand it to
// Sys_loadsf2. Returns TRUE if a soundfont was queued for loading.
// (Loading is async in the runtime; the next Sys_playmus that fires
// after the worklet is ready will use the SF2.)
LET try_load_sf2() = VALOF
{ LET listbuf = VEC 64
  LET namebuf = VEC 32
  ...
  { WHILE end <= totlen & listbuf % end ~= ',' DO end := end + 1
    IF ends_with_sf2(listbuf, start, end) DO
    { cp_substr(listbuf, start, end, namebuf)
      writef("soundfont: ")
      FOR i = start TO end - 1 DO wrch(listbuf % i)
      newline()
      IF sys(Sys_loadsf2, namebuf) ~= 0 RESULTIS TRUE
    }
    end := end + 1; start := end
  }
  RESULTIS FALSE
...
LET drawframe() BE
{ reset_clip()
  IF g_frame_count = 1 DO
    writef("drawframe: sky=%x8 floor=%x8 surf=%n root=%n px=%n py=%n*n",
           sky_col, floor_col, surf, g_root_node, px, py)
...
// ====================================================================
//
// All entry points below are exported via GLOBAL slots declared in
// g/doomengine.h. The library has no start(): consumers GET the
// header and drive the engine themselves.

// engine_init() — one-time setup: allocate caches, build trig tables,
// open SDL surface, init key state. Call exactly once at program
// start, BEFORE engine_load_wad_*. Returns TRUE on success.
LET engine_init() = VALOF
{ g_pal     := getvec(256)
  ...
  writef("engine_init: sky=%x8 floor=%x8 surf=%n*n", sky_col, floor_col, surf)

  keys := getvec(KEYCAP)
  FOR i = 0 TO KEYCAP DO keys!i := 0
  writef("engine_init: done*n")
  RESULTIS TRUE
}

// engine_load_wad_from_assets() — find the first .wad in the upload
// list, parse its header. Sets g_base / g_numlumps / g_dirofs.
// Returns TRUE on success.
LET engine_load_wad_from_assets() = VALOF
{ LET info = VEC 3
  UNLESS try_load_wad(info) DO
  { writef("doomengine: no WAD asset uploaded.*n")
    RESULTIS FALSE
  }
  IF info!1 ~= 0 DO
  { writef("doomengine: asset is an image, not a WAD.*n")
    RESULTIS FALSE
  }
  g_base     := info!2
...
// engine_load_map(map_name) — locate the BCPL-string named map marker
// (e.g. "E1M1"), parse all required lumps, composite textures + flats,
// load palette + sky, find player start. Pass 0 to use the first map
// marker in the WAD.
LET engine_load_map(map_name) = VALOF
{ LET map_idx = -1
  LET vert_idx, line_idx, side_idx, sec_idx = 0, 0, 0, 0
  LET node_idx, ssec_idx, seg_idx, thing_idx = 0, 0, 0, 0
  LET playpal_idx, pnames_idx, tex1_idx = -1, 0, 0
  LET things_byte, tsize = 0, 0
  LET side_sz, num_sides = 0, 0
  LET playpal_byte, pnames_byte = 0, 0

  TEST map_name = 0
  THEN map_idx := find_first_map(g_base, g_dirofs, g_numlumps)
  ELSE FOR i = 0 TO g_numlumps - 1 DO
       { LET e = g_dirofs + i * DIR_ENTRY_SIZE
         IF is_map_marker(g_base, e) & lump_name_eq(g_base, e, map_name) DO
         { map_idx := i; BREAK }
       }
  g_map_idx := map_idx
  IF map_idx < 0 DO { writef("doomengine: map not found.*n"); RESULTIS FALSE }
  ...
  { writef("doomengine: missing map lump.*n"); RESULTIS FALSE }
  ...
  { writef("doomengine: missing PLAYPAL / PNAMES / TEXTURE1.*n"); RESULTIS FALSE }
  ...
  { writef("doomengine: no player1 start.*n"); px := 0; py := 0; pa := 0 }
  ...
  { LET sky_name = VEC 2
    ...
    sky_name % 0 := 'S'; sky_name % 1 := 'K'
    sky_name % 2 := 'Y'; sky_name % 3 := '1'
  ...
  { ceil_h_runtime!s := sec_ceil_static(s)
    ...
    sec_light_state!s   := 0
    sec_light_timer!s   := next_rand() & 15
  }
  RESULTIS TRUE
}

// engine_load_default_sprites() — load weapon frames, HUD bar, face,
// number digits. Idempotent; safe to call once after engine_load_map.
LET engine_load_default_sprites() BE
{ writef("engine_load_default_sprites: enter*n")
  ...
  writef("sprites: PISGA0=%n STBAR=%n*n", wep_cidx_a, hud_cidx)
...
// engine_load_sf2_from_assets() — wrap try_load_sf2 for the consumer.
// Returns TRUE if a .sf2 was queued. The runtime loads it async; the
// next engine_start_music() will route through spessasynth once ready.
LET engine_load_sf2_from_assets() = try_load_sf2()

// engine_start_music() — try the current map's D_<name> MUS lump.
// Falls back to any uploaded .ogg / .mp3 / .wav asset.
LET engine_start_music() BE
{ UNLESS try_start_wad_mus() DO try_start_music()
}

// engine_stop_music() — silence current track.
LET engine_stop_music() BE sys(Sys_stopmusic)

// engine_poll_events() — pump input. Returns the running flag (TRUE
// until the SDL window receives a quit / Esc). Consumer's main loop
// should exit when this returns FALSE.
LET engine_poll_events() = VALOF
{ poll_events()
  RESULTIS running
}

// engine_key_down(k) — current state of an SDL keycode (K_* from sdl.h).
LET engine_key_down(k) = key_down(k)

// Player state accessors — read-only views of internal px/py/pa.
LET engine_player_x() = px
LET engine_player_y() = py
LET engine_player_a() = pa

// engine_walk(fwd) — collision-checked move along facing direction.
// fwd > 0 forward, fwd < 0 back. Magnitude scaled by ENGINE_MOVE
// (one step per unit fwd).
LET engine_walk(fwd) BE
{ LET dx = (cos_t!(pa & (ANG-1)) * MOVE * fwd) / 1024
  LET dy = (sin_t!(pa & (ANG-1)) * MOVE * fwd) / 1024
  try_move(dx, dy)
}

// engine_strafe(side) — collision-checked sideways move.
// side > 0 = right, side < 0 = left.
LET engine_strafe(side) BE
{ LET right_ang = (pa + ANG / 4) & (ANG - 1)   // +90°
  LET dx = (cos_t!right_ang * MOVE * side) / 1024
  LET dy = (sin_t!right_ang * MOVE * side) / 1024
  try_move(dx, dy)
}

// engine_turn(da) — rotate facing by da (ANG units, ANG=2048 == 360°).
LET engine_turn(da) BE pa := (pa + da) & (ANG - 1)

// engine_use() — single-shot "use" (open door etc.). Caller is
// responsible for edge-detection on the key.
LET engine_use() BE use_action()

// engine_fire() — start the weapon-fire animation. Caller edge-detects
// the trigger; one call kicks the timer through three frames.
LET engine_fire() BE wep_fire_timer := 3 * WEAPON_FIRE_TICKS

// engine_tick() — advance one game frame: doors, lights, anim counter,
// weapon bob phase, fire-frame countdown.
LET engine_tick() BE
{ tick_doors()
  tick_lights()
  g_frame_count := g_frame_count + 1
  IF wep_fire_timer > 0 DO wep_fire_timer := wep_fire_timer - 1
  // Bob phase advances faster while moving; simple proxy: any key down.
  TEST key_down(K_W) | key_down(K_S) | key_down(K_A) | key_down(K_D) |
       key_down(K_UP) | key_down(K_DOWN) | key_down(K_LEFT) | key_down(K_RIGHT)
  THEN wep_bob_t := (wep_bob_t + 96) & (ANG - 1)
  ELSE wep_bob_t := (wep_bob_t + 16) & (ANG - 1)
}

// engine_draw_frame() — render the current view (3D, sprites, HUD,
// automap overlay) and flip the surface.
LET engine_draw_frame() BE drawframe()

// engine_toggle_automap() — flip the automap overlay on/off.
LET engine_toggle_automap() BE
  automap_on := automap_on -> FALSE, TRUE

// engine_automap_zoom(delta) — change automap scale by delta * 16 q10
// units (delta > 0 = zoom in, < 0 = out). Clamped to a sane range.
LET engine_automap_zoom(delta) BE
{ automap_scale_q10 := automap_scale_q10 + delta * 16
  IF automap_scale_q10 <   32 DO automap_scale_q10 :=   32
  IF automap_scale_q10 > 2048 DO automap_scale_q10 := 2048
}

// engine_set_hud(health, ammo, armor) — update HUD numeric readouts.
LET engine_set_hud(h, a, ar) BE
{ hud_health := h
  hud_ammo   := a
  hud_armor  := ar
}
