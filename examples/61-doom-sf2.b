// 61-doom-sf2: 60 + SoundFont-backed MUS playback.
//
// What's new vs 60:
//   - Scan uploaded assets for the first name ending in ".sf2" and
//     load it via Sys_loadsf2 before starting music.
//   - The runtime converts the WAD's MUS lump to MIDI on the fly and
//     drives it through spessasynth using the loaded SoundFont, so
//     channels play with real instrument timbres instead of triangle
//     oscillators.
//   - With no SF2 asset present, behaviour is identical to 60.
//
// Drop a General-MIDI SoundFont (e.g. FluidR3_GM.sf2) into the Assets
// pane before running and you'll hear D_<map> with proper patches.
//
...
{ LET info = VEC 3
  ...
  { LET fwd  = 0
    ...
    { LET any_key = FALSE
      ...
      IF any_key DO
      { // Boot the SF2 player on the first user gesture (browsers
        // require one before AudioWorklet can decode). If a .sf2
        // asset is present the next Sys_playmus will route through
        // spessasynth once the worklet finishes loading; otherwise
        // we get the oscillator fallback as before.
        try_load_sf2()
