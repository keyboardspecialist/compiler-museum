// 52-doom-doors: 51 + door/lift open via the Use action.
//
// What's new vs 51:
//   - Press E (or Space) facing a door linedef. Linedefs with special
//     types 1 (DR), 31 (D1), 117 (DR, fast), 118 (D1, fast) open.
//   - Per-sector runtime ceiling height (ceil_h_runtime[]) overrides
//     the immutable WAD value.  sec_ceil() now reads the runtime cache,
//     so the renderer animates as the ceiling moves.
//   - Per frame, run a door state machine: opening sectors raise their
//     ceiling toward target at DOOR_SPEED, stop on contact.
//   - Target = (lowest neighbor ceiling) - 4 (Doom convention).
//
// Limitations:
//   - Close phase not modelled — open-and-stay only (functionally fine
//     for E1M1 starting doors). Locked doors (26/27/28) ignored.
//   - No use-line raycast: nearest-line-in-front-of-player heuristic.
//
...
// Build a BCPL string `name` = base[1..4] + 'A' + digit(1..8) + NUL.
// `buf` is a word vector with at least 8 bytes of room.
LET build_monster_name(base, octant, buf) BE
{ buf % 0 := 6
  buf % 1 := base % 1
  buf % 2 := base % 2
  buf % 3 := base % 3
...
{ LET n = g_tsize / THING_SIZE
  ...
  { LET o    = g_things_byte + i * THING_SIZE
    ...
    THEN { LET tang_t = (tang * ANG) / 360
           ...
           build_monster_name(base, oct, name_buf)
